...
- File naming: `[module].test.ts` / `[Component].test.tsx` under `test/` mirroring `src/` (single location — detection and delivery rely on `test/**`).
...
- **Do not** create a test suite when Phase 1 tests = No (and `/electron-run-tests` never scaffolds one unasked).
