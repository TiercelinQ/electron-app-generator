...
- `typescript ^6.0.0`: `moduleResolution: classic` removed (electron-vite uses `bundler` → no impact) and `esModuleInterop: false` forbidden (already true by default) — both fine here. **The binding constraint is `typescript-eslint`'s TypeScript peer range (`>=4.8.4 <6.1.0` for v8, see below): pin TypeScript inside it.** TS **6.0.x shipped stable** and is supported; the registry `latest` is now **7.x** (native/Go-port line), which typescript-eslint 8 does **not** support — so pin `^6.0.0`, not `latest` / `^7`, until a typescript-eslint major widens the range. Re-verify at generation (`npm view typescript dist-tags`, `npm view typescript-eslint peerDependencies`) — the numbers move.
...
- **Tests (if enabled in Phase 1)** — add to `devDependencies`: `vitest ^4.1.0`, `@testing-library/react ^16.3.0`, `@testing-library/jest-dom`, `jsdom ^29.0.0`, plus the `"test": "vitest run"` script. Detail: `@rules/tests.md`.
- **`electron-log ^5.4.0`**: mandatory in every project (file logging) — see `@rules/logging.md`.

> **Version maintenance**: this table and these notes reflect the state validated at the time of writing and **age quickly** — treat every version number **and every version-history claim** above as *indicative*, not authoritative. Before pinning in a new project, re-confirm the current **minor and major** versions and the breaking-change / peer-range notes (`npm outdated`, `npm view <pkg> dist-tags`, the release notes). The caret rule and the structure stay; the numbers and notes are refreshed per project in Phase 1.
