...
- Tests are opt-in at Phase 1. If enabled, the suite (Vitest + Testing Library) was delivered in its dedicated batch — run it (`npm test`).
- **Do not create a test suite if the project has none** (Phase 1 tests = No). The default verification is typecheck + lint + build.
